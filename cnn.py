...
# using a 2 fc layers NN to represent the t-SNE
# model_pred = [-1, 10]
model_pred = tf.matmul(h_fc1_drop, W_fc2) + b_fc2

# ********************* FC1 layer *********************************
W_fc3 = weight_variable([10, 10])
b_fc3 = bias_variable([10])
h_fc3 = tf.nn.relu(tf.matmul(model_pred, W_fc3) + b_fc3)

# ********************* FC2 layer *********************************
W_fc4 = weight_variable([10, 2])
b_fc4 = bias_variable([2])
h_fc4 = tf.nn.relu(tf.matmul(h_fc3, W_fc4) + b_fc4)

KL_loss = tSNE.tsne_loss(model_pred, h_fc4, no_dims=2, perplexity=30.0)
train2_step = tf.train.AdamOptimizer(1e-4).minimize(KL_loss)

for i in range(1000):
    batch_x, batch_y = mnist.train.next_batch(100)
    sess.run(train2_step, feed_dict={x: batch_x, y: batch_y, keep_prob: 0.8})


print("Run Y = tsneNN(X, no_dims,perplexity) to use NN to perform t-SNE on mnist dataset.")
print("Running example on 10,000 MNIST digits...")
X, Labels = mnist.test.images, mnist.test.labels
Y = sess.run(h_fc4, feed_dict={x: X, y: Labels, keep_prob: 1.0})
